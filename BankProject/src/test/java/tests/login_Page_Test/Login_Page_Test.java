...
import componentFunctions.DataProviderClass;
...
public class Login_Page_Test extends BaseClass {
  ...
  @Test(priority = 0)
	...
	public void successfullLoginWithParameters(@Optional("Sangram") String username,String password){
		loginPV = new Login_Page_Verification(driver);
		loginPV.verifyLoginWithParameters(username,password);
	}
	
	@Test(priority = 5,dataProvider="credentials",dataProviderClass=DataProviderClass.class)
	public void successfullLoginWithDataProvider(String username,String password){
		loginPV = new Login_Page_Verification(driver);
		loginPV.verifyLoginWithParameters(username,password);
	}
}
